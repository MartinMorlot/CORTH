...
i= 1
pdf("Tes_selection_check_comprehensive_grass_new.pdf")
for(file_to_read in station_files){
  ...
  serie_data[interval_data] <- na.approx(serie_data[interval_data], maxgap = 365)
  ...
  max_serie <- aggregate(serie_data, by=list(tS_d), FUN=max, na.rm=T)
  
  min_serie <- aggregate(serie_data, by=list(tS_d), FUN=min, na.rm=T)
  
  median_serie <- aggregate(serie_data, by=list(tS_d), FUN=median, na.rm=T)
  
  q1090_serie <- aggregate(serie_data, by=list(tS_d), FUN=quantile, c(0.1,0.9), na.rm=T)
...
column_for_rem <- which(colnames(dataFrame_anomaly_data) %in% column_name_grass)

dataFrame_anomaly_data <- dataFrame_anomaly_data[,-column_for_rem]

column_for_rem_normal <- which(colnames(dataFrame_normalized_data) %in% column_name_grass)
...
matplot(dataFrame_normalized_data$date[which(format(dataFrame_normalized_data$date, "%Y") > 2020)],
        ...
        type='l', ylab='DeltaH', xlab='Date', ylim=c(-1,0))

data_anomaly_post <- dataFrame_anomaly_data[which(format(dataFrame_anomaly_data$date, "%Y") >= 2022),]

data_normalized_post <- dataFrame_normalized_data[which(format(dataFrame_normalized_data$date, "%Y") >= 2022),]
...
reduced_data_normal <- subset(data_normalized_post, select = which(na_counts < 500))
reduced_data_anomaly <- subset(data_anomaly_post, select = which(na_counts < 500))
...
ggcorrplot(corr_matrix_normal, type = "lower",
           lab = TRUE)
# Compute and show the  result
ggcorrplot(corr_matrix_anomaly, type = "lower",
           lab = TRUE)
...
filtered_cor_matrix <- ifelse(close_stations, corr_matrix_anomaly, NA)

colnames(filtered_cor_matrix) <- colnames(corr_matrix_anomaly)
rownames(filtered_cor_matrix) <- rownames(corr_matrix_anomaly)
...
pdf("Comparison_Correlations_anomaly_over_04_close_test_new.pdf")
object_count <- 1
for(col in colnames(filtered_cor_matrix)){
  data <-filtered_cor_matrix[,col]
  rm_slot <- which(names(data) == col)
  good <- names(which(data[-rm_slot] >= 0.4))
  if(length(good) > 0){
    for(stat in good){
      line <- rbind(
        c(
...
writeVector(stations_sel, "station_good_for_correlation_anomaly_test_grass.gpkg", overwrite=T)
...
writeVector(good_cor_lines, "good_correlation_anomaly_test_grass.gpkg", overwrite=T)
