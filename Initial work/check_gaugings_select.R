...
sel_station <- !is.na(stations$ratio_gauging) & (stations$ratio_gauging > 0.4)

nrow(stations[sel_station])
# 
# sel_station <- !is.na(stations$ratio_gauging) & (stations$ratio_gauging > 0.3)
# 
# nrow(stations[sel_station])

writeVector(stations[sel_station], "Test_stations_selected_gauging_over04.gpkg", overwrite=T)
